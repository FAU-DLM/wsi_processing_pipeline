...
class GuidedGradCam:
    ...
    def generate_guided_grad_cam(self, input_image:torch.Tensor, class_index:int, normalize:bool)->torch.Tensor:
        """
        Arguments:
            ...
            class_index: e.g. you got four classes [A,B,C,D] class_index can be one of the numbers
                         0, 1, 2, 3
            normalize: if True, the mask gets denormed
        Return:
        ...
        if(normalize):
            guided_grad_cam_mask = guided_grad_cam_mask - guided_grad_cam_mask.min()
            guided_grad_cam_mask /= guided_grad_cam_mask.max()
